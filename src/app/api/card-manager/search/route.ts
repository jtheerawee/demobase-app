...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (scanIds) {
            ...
            const { data, error } =
                await supabaseQuery.limit(100);
            if (error) {
                console.error(
                    "[API Search] Supabase Error:",
                    error,
                );
                return NextResponse.json(
                    {
            ...
            const cards = filteredCards.map(
                (card: any) => ({
                    ...
                    rarity: card.rarity,
                    collectionName:
                        card.scraped_collections?.name,
                    collectionCode:
                        card.scraped_collections
                            ?.collection_code,
                    franchise:
                        card.scraped_collections?.franchise,
                }),
            );
        ...
        if (error) {
            console.error(
                "[API Search] Error fetching cards:",
                error,
            );
            return NextResponse.json(
        ...
        const cards = data.map((card: any) => ({
            ...
            collectionCode:
                card.scraped_collections?.collection_code,
