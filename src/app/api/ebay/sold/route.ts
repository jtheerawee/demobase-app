...
export async function GET(request: NextRequest) {
    ...
    if (sp.has("grade"))
        forward.set("grade", sp.get("grade")!);
    if (sp.has("service"))
        forward.set("service", sp.get("service")!);
    if (sp.has("page"))
        forward.set("page", sp.get("page")!);
    if (sp.has("minPrice"))
        forward.set("minPrice", sp.get("minPrice")!);
    if (sp.has("maxPrice"))
        forward.set("maxPrice", sp.get("maxPrice")!);

    // camelCase → snake_case for exclude/us flags
    if (
        sp.get("excludeJp") === "true" ||
        sp.get("exclude_jp") === "true"
    )
