...
// ==========================================
// ONE PIECE JAPANESE (JP) COLLECTION SCRAPER
// ==========================================

export async function scrapeOnepieceCollectionsJp({ url, context, send, franchise, language }: ScraperOptions) {
    send({ type: "step", message: "Discovering One Piece JP collections..." });
    ...
    try {
        ...
        const collections = await workerPage.evaluate(() => {
            ...
            return Array.from(listItems).map((el) => {
                ...
                // Construct URL: original base + ?series=value (or replace if exists)
                ...
                return {
                    ...
                    imageUrl: "", // Icons aren't usually in this list
                    collectionUrl,
                };
        ...
        if (franchise && language && collections.length > 0) {
            send({ type: "step", message: `Found ${collections.length} One Piece JP collections.` });
            ...
            if (result) {
                ...
                send({ type: "step", message: `Successfully registered ${collections.length} One Piece JP collections — ✅ ${added} new, 🔁 ${matched} matched.` });
            }
        } else {
