...
export async function scrapePokemonCollectionsEn({
    ...
    try {
        ...
        send({ type: "step", message: "Activating Advanced Search filters..." });
        ...
        await page.waitForSelector(advancedBtnSelector, { timeout: 15000 }).catch(() => { });
        ...
        send({ type: "step", message: "Expanding the 'Expansions' section..." });
        // 2. Click "Expansions" header
        const expansionHeaderSelector = "header:has-text('Expansions'), div.column-12:has-text('Expansions'), .expansion-filter header";
        send({ type: "step", message: "Waiting for Expansions section..." });
        await page.waitForSelector(expansionHeaderSelector, { timeout: 15000 }).catch(() => {
            console.warn("Expansion header selector timeout");
        });

        const expansionSection = await page.$(expansionHeaderSelector);
        if (expansionSection) {
            send({ type: "step", message: "Clicking Expansions section..." });
            await expansionSection.scrollIntoViewIfNeeded().catch(() => { });
            await expansionSection.click({ force: true, timeout: 5000 }).catch((e: any) => {
                console.warn("Expansion click failed", e);
            });
            await page.waitForTimeout(1000);
        } else {
            send({ type: "step", message: "Warning: Expansions section not found, proceeding anyway..." });
        }

        send({ type: "step", message: "Scraping set names from filters..." });
