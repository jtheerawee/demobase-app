                ...
                try {
                    ...
                    const pageData = await workerPage.evaluate(() => {
                        ...
                        const items = Array.from(collectionElements).map((el) => {
                            ...
                            const expansionCodeMatch = absoluteLink.match(/expansionCodes=([^&]+)/);
    ...
    if (franchise && language && sharedCollectionList.length > 0) {
        ...
        try {
            ...
            if (result) {
                ...
                logStep(
                    `Successfully saved ${sharedCollectionList.length} collections — ✅ ${addedItems.length} new, 🔁 ${matchedItems.length} matched.`,
                );
            }
        } catch (error) {
