...
export async function scrapeMTGCards({
    ...
    send({
        type: "step",
        message:
            "MTG Gatherer detected. Starting card extraction...",
    });
    let activeWorkers = 0;
    let discardedCount = 0;

    const isMismatchedSlug = (
        name: string,
        url: string,
    ) => {
        try {
    ...
    const isModernSetUrl =
        /gatherer\.wizards\.com\/sets\/[A-Za-z0-9]+/.test(
            url,
        );
    ...
    try {
        if (isModernSetUrl) {
            ...
            while (true) {
                ...
                const pageCards = rawPageCards.filter(
                    (c: any) => {
                        const mismatched = isMismatchedSlug(
                            c.name,
                            c.cardUrl,
                        );
                        if (mismatched) {
        ...
        if (deepScrape && allCards.length > 0) {
            const skipCount =
                allCards.length - cardsToDeepScrape.length;
            if (skipCount > 0) {
                send({
                    type: "step",
            ...
            if (cardsToDeepScrape.length > 0) {
                ...
                if (
                    ...
                    send({
                        type: "step",
                        message:
                            "Finalizing card details in database...",
                    });
                    try {
                        const result =
                            await saveScrapedCards(
